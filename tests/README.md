...
- All service health checks pass and the frontend can reach the backend database
  readiness endpoint through its same-origin proxy.
...
- Database failure returns readiness HTTP 503 while backend liveness remains 200,
  and readiness recovers after PostgreSQL returns.
